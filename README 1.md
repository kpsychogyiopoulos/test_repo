...
- __PT_20260301_v2.zip__ : This project asset is deployed within the CP4D persistent volume. It contains raw CSV inputs, intermediate Parquet files generated by the code, and finalized model embeddings (.pkl).
- __inference_pkl.zip__: This asset contains pkl files that were unzipped inside PT_20260301_v2/ necessary for the inference pipeline execution.
- data_transf.zip
...
```
os.environ["date"] = "PT_<YYYY-MM-DD>"
os.environ["phase"] = "<phase>" #train/inference
os.environ["current_date"] = "<YYYY-MM-DD>"
```
...
Having setup and initiated the spark session that we will be using, we inject the it to the Kedro pipeline.
...
- Edited the following yml files to include the correct paths to the new data sources:
  ...
  Edited the following source code files to process correctly the data files and parameters read:      
  🛠️ src/hedno_losses/pipelines/preprocessing/nodes/consumptions.py (line 245):
  - Function: filter_consumptions()
    - We refactored the code so that we ensure that a date provided as text is converted into a functional date object so the pipeline can safely perform time-based calculations.
